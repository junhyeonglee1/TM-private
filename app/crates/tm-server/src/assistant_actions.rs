...
use tm_core::{
    ...
    AssistantActionStatus, Error as CoreError, Task,
};
...
pub(super) async fn approve(
    ...
    require_confirmation(&headers, "task.create", &request_id)?;
    let idempotency_key =
...
fn action_dto(
    ...
    let result = action
        ...
        .map(|result| {
            let task: Task =
                serde_json::from_value(result.entity).map_err(|_| response_error(request_id))?;
            let item = serde_json::to_value(TaskDto::from(task))
    ...
    Ok(ActionDto {
        ...
        approval: ApprovalContract {
            ...
            required_confirmation: "task.create",
