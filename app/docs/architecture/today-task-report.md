...
사용자가 Windows `오늘` 화면 또는 모바일 PWA에서 명시적으로 버튼을 누를 때만 OpenAI Responses API를 한 번 호출한다. Task 우선순위와 오늘부터 7일 이내의 개인 일정·납부일을 함께 보고한다. 자동 스케줄, push, email, Task·일정 변경은 이번 범위에 없다.
...
Task는 중복을 제거한 최대 15개를 사용한다. 캘린더는 서울 날짜 기준 오늘부터 7일 이내 occurrence를 계산해 최대 5개를 사용하므로 전체 후보 상한은 기존과 같은 20개다. OpenAI 입력 allowlist는 Task ID·제목·프로젝트 이름·상태·우선순위·마감일·선정 category와 캘린더 occurrence ID·event ID·제목·종류·날짜·시간·반복 종류다. Task 설명, 캘린더 메모, checklist, Note, Worklog, Session, memory, attachment, 인증·backup·감사·비용 원장은 전달하지 않는다. 제목과 프로젝트 이름을 포함한 모든 값은 명령이 아닌 신뢰하지 않는 데이터로 prompt에 선언한다.
...
- prompt version: `calendar-task-report-v1`

하루 제한 claim과 실행 record 생성은 `BEGIN IMMEDIATE` transaction 하나에서 처리한다. 시작 뒤 실패하거나 서버가 재시작된 시도도 당일 상한에 포함해 과금 폭주를 fail closed한다. 열린 Task와 7일 이내 일정이 모두 없을 때만 OpenAI를 호출하거나 일일 AI 시도를 소비하지 않고 비용 0의 결정론적 결과를 기록한다.

## 결과 검증

Responses API Structured Outputs의 strict JSON Schema로 headline, summary, Task가 있으면 1~3개 priority, 일정이 있으면 1~5개 schedule highlight와 최대 3개 alert를 받는다. 서버가 다시 다음 항목을 검증한다.

- priority Task ID가 입력 후보에 실제로 존재한다.
- schedule occurrence ID가 입력 후보에 존재하고 event ID·제목·종류·날짜·시간이 원문과 정확히 일치한다.
...
일정 생성·수정·삭제, push 알림, 정기 지출 자동화, 주식은 각각 별도 data selector·prompt·endpoint·비용/권한 정책과 사용자 승인을 가진 기능으로 확장한다.
