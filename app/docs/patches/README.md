# 패치 기록

패치 문서는 구현 이유, 실제 변경 범위, DB 영향, 검증 결과와 복구 방법을 함께 남긴다.

- `in-progress`: 구현 또는 검증이 남아 있음
- `verified`: 문서에 적힌 전체 검증과 release 산출물 확인이 완료됨

문서 번호는 네 자리 숫자로 증가시키며 기존 패치 문서를 덮어쓰지 않는다.

- `0001-local-first-foundation.md`: 초기 local-first 앱 기반 (`verified`)
- `0002-project-task-workflow.md`: 프로젝트별 Task 작업공간과 Inbox 분류 흐름 (`in-progress`)
- `0003-manual-change-request-queue.md`: 수동 승인 개선 요청 큐 (`in-progress`)
- `0004-change-request-writing-guide.md`: 개선 요청 유형별 작성 가이드 (`in-progress`)
- `0005-compact-project-task-flow.md`: Inbox 보류와 compact 프로젝트 Task 생성 흐름 (`in-progress`)
- `0006-production-embedded-frontend.md`: release 내장 프런트엔드 강제와 localhost 오류 수정 (`in-progress`)
- `0007-cloud-change-request-processing.md`: Railway 개선 요청 claim·완료·실패 연결 (`verified`)
- `0008-one-touch-task-completion.md`: Task 목록 원터치 완료와 확인 모달 (`verified`)
- `0009-hide-cloud-credential-console.md`: Windows 탭 전환 시 콘솔 창 깜박임 제거 (`verified`)
