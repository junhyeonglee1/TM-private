...
- [ ] 초기 버전을 본인 한 명만 사용하는 단일 사용자 시스템으로 확정
- [ ] 인증 방식 선택: 관리형 로그인, passkey 또는 장기 토큰
- [ ] 새 기기 등록과 분실 기기 해제 방식 결정
- [ ] 세션 만료 시간 결정

Codex Action Item:

- [ ] 모든 비공개 API에 인증 미들웨어 적용
- [ ] 서버에 원문 인증 토큰을 저장하지 않고 해시만 저장
- [ ] 로그인 시도·API 요청 rate limit 적용
- [ ] CORS·CSRF·보안 헤더 설정
- [ ] 인증 실패 로그에서 민감정보 제거
- [ ] 키·토큰 회전과 폐기 절차 작성
- [ ] 인증 완료 후에만 HTTPS 공개 도메인 생성
...
1. Codex가 해당 단계의 목적과 사용자 결정사항을 설명한다.
2. 사용자가 계정·비용·접근 권한·자동화 수준을 결정한다.
3. Codex가 코드와 문서를 구현한다.
4. 자동 테스트와 안전 검사를 통과한다.
5. 사용자가 실제 환경에서 동작을 확인한다.
6. 로컬 Git 커밋으로 체크포인트를 만든다.
