...
- 기본 프로젝트: 프로젝트를 선택하지 않으면 실제 시스템 `기타` 프로젝트로 저장
...
프로젝트 편집·보관·삭제와 Task 삭제는 이번 모바일 범위에 포함하지 않는다. schema 14 migration은 기존 활성 `기타`를 시스템 기본 프로젝트로 채택하고 프로젝트가 없던 Task를 그 ID로 이관한다. 시스템 `기타`는 이름 변경·보관·휴지통 이동·삭제할 수 없다.
...
- service worker의 cache 이름은 `tm-mobile-shell-v12`이다. 활성화할 때 이전 shell cache를 제거한다.
...
1. 배포 직전에 encrypted remote backup이 schema 13, migration ledger 1..13, integrity `ok`인지 확인한다. 동일 Railway volume의 자동 pre-migration backup만 rollback 근거로 사용하지 않는다.
2. 아래 baseline capture를 실행해 활성 프로젝트 수, 기존 `기타` ID, 미지정 Task의 ID·비내용 필드와 제목·설명 SHA-256을 로컬 `dist`에 저장한다. 원문·token은 저장하지 않는다.
3. 검증된 소스를 GitHub에 push한다.
4. `STEP 10 Windows build`와 `STEP 16 security`의 성공 run ID를 기록한다.
5. Windows artifact의 두 실행 파일 SHA-256을 기록한다.
6. Task·프로젝트 쓰기를 잠시 멈춘 뒤 같은 검증 commit을 Railway production에 배포한다.
7. `/readyz`가 `200`이고 production DB와 원격 백업이 모두 schema 14이며 backup 상태가 `succeeded`, integrity가 `ok`, `schemaSemanticsValidated=true`인지 확인한다.
8. baseline을 전달해 아래 비파괴 production verifier를 실행한다.
9. 실제 등록 모바일 기기에서 한 번 새로고침한다. 이전 PWA가 열려 있으면 v12 service worker가 활성화되도록 앱을 완전히 닫았다가 다시 열 수 있다.
10. 모바일에서 프로젝트 추가, Task 추가, 상태 변경, 빠른 완료를 사용자 승인 아래 각각 한 번 수행하고 Windows 앱에 같은 결과가 보이는지 확인한다.

schema 14 DB는 schema 13 binary로 열 수 없다. 이 전환 뒤 이전 Railway image 재배포를 rollback으로 사용하지 않으며, 실패는 schema 14 수정본으로 roll-forward한다. 정말 복원이 필요하면 새 쓰기가 시작되기 전에 검증된 off-volume schema 13 backup을 복구하고 그 이후 쓰기 손실을 명시적으로 승인해야 한다.

```powershell
Set-Location 'C:\Users\tkfk0\Desktop\codex\TM\app'
$baselineScript = '.\scripts\capture-uncategorized-migration-baseline.ps1'
$baselinePath = '..\dist\manual-uncategorized-migration-baseline\baseline.json'
& $baselineScript -OutputPath $baselinePath

# GitHub Actions 검증과 Railway schema 14 배포가 끝난 뒤 실행한다.
$verifyScript = '.\scripts\verify-mobile-project-tas' + 'k-production.ps1'
& $verifyScript -MigrationBaselinePath $baselinePath
```
...
baseline에는 기존 프로젝트·Task ID와 개수, 비내용 필드, 제목·설명 SHA-256만 저장하며 원문과 secret은 저장하지 않는다. verifier 결과에는 bearer token, pairing code, polling secret, device·CSRF cookie, 기기 ID, 프로젝트·Task 원문 또는 그 개수를 저장하지 않는다.
...
3. service worker가 `tm-mobile-shell-v12`이고 API·non-GET을 cache하지 않는지 확인한다.
...
10. 전후 operations의 DB schema·원격 백업 상태/schema/integrity/schema semantic guard와 OpenAI API 비용 원장이 같은지 확인한다.

schema 14 배포 검증에서는 schema 13 baseline과 대조해 기존 활성 `기타` ID와 프로젝트 수가 유지됐는지, read API에 노출되는 모든 Task의 `projectId`가 채워졌는지, 이전 미지정 Task의 제목·설명·상태·우선순위·기한·완료 시각·생성/수정 시각은 유지되고 version만 정확히 한 번 증가했는지 확인한다. soft-deleted Task까지 포함한 전체 DB 이관은 schema 13→14 migration 통합 테스트와 schema 14 semantic validator로 별도 검증한다.
